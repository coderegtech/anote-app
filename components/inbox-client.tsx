...
export default function InboxClient({
  ...
  const saveQuestion = async (q: string) => {
    try {
  ...
  const EditableQuestion = () => {
    const handleBlur = () => {
      ...
      saveQuestion(text || "Send me anonymous question!")
      setIsEditing(false)
    }
    ...
    return (
      <div className="space-y-4 max-w-md mx-auto">
        {messages.map((msg) => (
          ...
          >
            {/* notes */}
            <span className="text-base font-semibold">Note: {msg.note}</span>
  ...
  const PlaySection = () => (
    <div className="space-y-6 max-w-md mx-auto pb-8">
      <Card className="bg-neutral-500 border-0 shadow-xl overflow-hidden rounded-2xl">
      ...
      <div className="space-y-4 px-6 bg-neutral-100 rounded-2xl py-8">
        <div className="text-center space-y-3">
          ...
          <Button
            ...
            variant="outline"
            size="lg"
            className="rounded-xl border-2 border-black text-black hover:bg-black hover:text-white font-semibold bg-transparent w-full"
          >
  ...
  return (
    <div className="mobile-full-height min-h-screen bg-white flex flex-col max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 px-4 py-3 sticky top-0 bg-white z-10">
        <div className="flex gap-x-3 overflow-x-auto scrollbar-hide">
          <button
            ...
            className={`text-base sm:text-lg font-bold whitespace-nowrap pb-1 ${
              activeTab === "play" ? "text-black border-b-2 border-black" : "text-gray-400"
            }`}
          >
            PLAY
          </button>
          <button
            ...
            className={`text-base sm:text-lg font-bold whitespace-nowrap pb-1 ${
              activeTab === "inbox" ? "text-black border-b-2 border-black" : "text-gray-400"
            }`}
          >
            INBOX
          </button>
          <button
            ...
            className={`text-base sm:text-lg font-bold whitespace-nowrap pb-1 ${
              activeTab === "chat" ? "text-black border-b-2 border-black" : "text-gray-400"
            }`}
          >
            CHAT
          </button>
          <button
            ...
            className={`text-base sm:text-lg font-bold whitespace-nowrap pb-1 ${
              activeTab === "questions" ? "text-black border-b-2 border-black" : "text-gray-400"
            }`}
          >
            QUESTIONS
          </button>
        ...
        <Button variant="ghost" size="icon" className="text-gray-400 hover:text-black shrink-0" onClick={handleSignOut}>
